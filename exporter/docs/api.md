...
* (optional) **types** — boolean flag to write YT column types as the second Excel row; default=true
...
The first row contains the names of the columns of the table.
If **types=true** (default), the second row contains YT types and data starts from the third row.
If **types=false**, data starts from the second row.
The header (and the table) contains only the requested columns.

example output as csv (`types=true`):
```
id,action
uint64,string
31322749,START
...
example output as csv (`types=false`):
```
id,action
31322749,START
31324254,PAGE_SWITCH
...
example:
```
&query_id=abcd&result_index=0&lower_row_index=1&upper_row_index=2&columns=col1&columns=col2&columns=col3&filename=output_file&number_precision_mode=lose&types=false
```
### Response
